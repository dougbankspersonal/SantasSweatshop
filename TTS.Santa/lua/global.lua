...
-- Season
local nullSeasonNumber = -1
-- Init currentRound to say "we are not playing yet".
local currentSeason = nullSeasonNumber
...
-- UI
local dealNextRoundButtonId = "DealNextRoundButton"
...
--[[
Utilities, nice for any/all game.
...
local function updateHostButtons()
    if currentSeason == nullSeasonNumber or currentSeason >= numSeasons then
        UI.setAttribute(dealNextRoundButtonId, "active", false)
        UI.setAttribute(dealNextRoundButtonId, "active", false)
    else
        UI.setAttribute(dealNextRoundButtonId, "active", true)
        UI.setAttribute(dealNextRoundButtonId, "text", "Start Round " .. (currentSeason + 1))
    end
end

local function onLoadUI()
    updateHostButtons()
end
...
local function placeCardWhichMayChangeDeck(deck, rowIndex, cardIndex, cardsToDeal, callback, opt_options)
    local options = opt_options or {}

    local flip
    if options.hideLastCard and cardIndex >= (cardsToDeal - 1) then
        flip = false
    else
        flip = true
    end
    ...
    if options.cardTwiddleCallback then
        options.cardTwiddleCallback(card, cardIndex)
    else
    ...
    local xPos = -rowWidth/2 + (rowWidth / cardsToDeal) * cardIndex
    local zPos = (numSeasons/2 * columnHeight) - (columnHeight * rowIndex)
    print("Doug: rowIndex = ", rowIndex)
    print("Doug: zPos = ", zPos)
...
local function recursivePlaceNextCard(deck, rowIndex, cardIndex, cardsToDeal, finalCallback, opt_options)
    -- Exit step: hit the final callback.
    if not deck or cardIndex >= cardsToDeal then
        finalCallback()
        return
    end

    placeCardWhichMayChangeDeck(deck, rowIndex, cardIndex, cardsToDeal, function(updatedDeck)
        -- Wait a bit then deal the next card.
        Wait.time(function()
            cardIndex = cardIndex + 1
            recursivePlaceNextCard(updatedDeck, rowIndex, cardIndex, cardsToDeal, finalCallback, opt_options)
...
local function placeCardsForSeason(seasonIndex, callback)
    ...
    elseif currentSeason == numSeasons then
        cardsThisSeason = cardsThisSeason + playerCount
    end

    local gameDeck = getObjectFromGUID(gameDeckGUID)

    recursivePlaceNextCard(gameDeck, 0, 0, cardsThisSeason, function()
        -- All done dealing.  Anything to do here?
    end, {
...
local function cleanupOldGame()
    ...
    -- Set round indicator to nil.
    currentSeason = nullSeasonNumber
...
local function maybeMakeSourceDecksFromImportedDeck(callback)
    ...
    recursivePlaceNextCard(importedDeck, -1, 0, importedCardCount, function()
        Wait.time(function()
...
function setup(clickedPlayer)
    ...
    -- Set current round to 0: game is playing but no round started yet.
    currentSeason = 0
...
function dealNextRound()
    if currentSeason == nullSeasonNumber then
        -- We are not even playing.
        return
    end

    -- We are starting a round.
    currentSeason = currentSeason + 1
    updateHostButtons()

    if currentSeason <= numRounds then
        placeCardsForThisRound()
    end
end
